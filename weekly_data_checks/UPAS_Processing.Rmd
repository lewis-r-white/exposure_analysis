...
path <- "/Users/lewiswhite/CHAP_columbia/GRAPHS/exposure_analysis/weekly_data_checks/upas_data2025/weekly_report_data/ASANTEKWAA UPAS DATA_20251206"
...
function_read_UPAS_data_ <- function(filepath) {
  ...
  # Fix DateTimeLocal (accepts both "2025-11-24 10:58:00" and "2025-11-24T10:58:00")
  if ("DateTimeLocal" %in% names(data)) {
    data$DateTimeLocal <- suppressWarnings(lubridate::ymd_hms(gsub("T", " ", data$DateTimeLocal), tz = "UTC"))
  }
  
  # Fix DateTimeUTC
  if ("DateTimeUTC" %in% names(data)) {
    data$DateTimeUTC <- suppressWarnings(lubridate::ymd_hms(gsub("T", " ", data$DateTimeUTC), tz = "UTC"))
  }
  
  # Fix UnixTime
  if ("UnixTime" %in% names(data)) {
    data$UnixTime_POSIX <- suppressWarnings(as.POSIXct(
      as.numeric(data$UnixTime),
      origin = "1970-01-01",
      tz = "UTC"
    ))
  }
  
  # Unified timestamp column (prefer local → UTC)
  data$timestamp <- dplyr::coalesce(data$DateTimeLocal, 
                                        data$UnixTime_POSIX, 
                                        data$DateTimeUTC)
  
  data$.__ts <- data$timestamp
...
for(i in 1:length(file_upas)){
  ...
  list.upas[[i]] <- function_file %>% 
    mutate(filter_id   = str_extract(file_upas[i], "KHU[0-9]{4}"),
      ...
      ## SULE commenting out lines below because changed how we clean the datetim above. 
...
pm_summary_upas <- data.frame(
  ...
  start_time = as.POSIXct(rep(NA, length(list.upasr3_new)), origin = "1970-01-01", tz = "UTC"),
  end_time   = as.POSIXct(rep(NA, length(list.upasr3_new)), origin = "1970-01-01", tz = "UTC"),
...
for (i in 1:length(list.upasr3_new)) {
  tryCatch({
    ## get date info
    ## SULE commenting out because already formatted
    ...
    # SULE
    # commenting out below because ordering lead to NAs at end which broke end_time column
    # list.upasr3_new[[i]] <- list.upasr3_new[[i]][order(list.upasr3_new[[i]]$DateTimeLocal), ]
    
    #create temp df for ease
    df <- list.upasr3_new[[i]]
    
    df <- df[!is.na(df$DateTimeLocal), ]
    df <- df[order(df$DateTimeLocal), ]
    
    list.upasr3_new[[i]] <- df
    # SULE 
  ...
  pm_summary_upas$start_time[i] <- df$DateTimeLocal[1]
  pm_summary_upas$end_time[i]   <- df$DateTimeLocal[nrow(df)]

  #pm_summary_upas$start_time[i] <- list.upasr3_new[[i]]$DateTimeLocal[1]
  #as.Date(list.upasr3_new[[i]]$DateTimeLocal[1])
  
  # pm_summary_upas$end_time[i]   <- list.upasr3_new[[i]]$DateTimeLocal[nrow(list.upasr3_new[[i]])]
  # commenting out bc calc end time differently now
  # pm_summary_upas$end_time[i] <-  list.upasr3_new[[i]]$DateTimeLocal[nrow(list.upasr3_new[[i]])] 
  
  # SULE
  
  #as.Date(list.upasr3_new[[i]]$DateTimeLocal[nrow(list.upasr3_new[[i]])])
